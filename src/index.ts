...
class Context {
  ...
  extend(symbol: Symbol | string, sep = '^') {
    const nm = typeof symbol == 'string' ? symbol : symbol.name;
    return new Context(
      this.tc,
      this.exports,
      this.basedir,
      this.id ? this.id + sep + nm : nm,
      this.typeParams
    );
  }
  ...
  addParams(typeParams: Param[]) {
    return new Context(
      ...
      this.basedir,
      this.id,
      typeParams.concat(this.typeParams)
    );
  }

  gatherSymbols(
    symbols: readonly Symbol[],
    target: { [name: string]: Item } = {},
    sep = '.',
    filter?: (name: string, value: Item) => boolean
  ) {
    let gathered = 0;
    for (const symbol of symbols
      .filter((s) => maybeDecl(s))
      .sort(compareSymbols)) {
      const name = this.symbolName(symbol);
      const item = this.extend(name, sep).itemForSymbol(symbol);
  ...
  symbolName(symbol: Symbol) {
    ...
    const name = symbol.name.slice(3).match(/^[^@]*/)![0];
  ...
  itemForSymbol(symbol: Symbol, kind?: BindingKind): Item | null {
    if (kind) {
      ...
      const aliased = this.tc.getAliasedSymbol(symbol);
      ...
      throw new Error(
        `Can not determine a kind for symbol ${symbol.escapedName} with flags ${symbol.flags}`
      );

    let binding: Binding = { kind, id: this.id },
      type = this.symbolType(symbol);
    this.addSourceData(symbol.declarations || [], binding);

    const mods = symbol.valueDeclaration
      ? getCombinedModifierFlags(symbol.valueDeclaration)
      : 0;
    if (mods & ModifierFlags.Abstract) binding.abstract = true;
    if (
      mods & ModifierFlags.Readonly ||
      (symbol.flags & (SymbolFlags.GetAccessor | SymbolFlags.SetAccessor)) ==
        SymbolFlags.GetAccessor
    )
      binding.readonly = true;
    if (
      mods & (ModifierFlags.Private | ModifierFlags.Protected) ||
      (binding.description && /@internal\b/.test(binding.description))
    )
      return null;
    ...
    let cx: Context = this;
    const params = ItemsWithParams.includes(binding.kind)
      ? this.getTypeParams(decl(symbol))
      : null;
    if (params) cx = cx.addParams(params);
    const typeDesc =
      kind == 'enum'
        ? cx.getEnumType(symbol)
        : kind == 'reexport'
        ? cx.getReferenceType(this.tc.getAliasedSymbol(symbol))
        : cx.getType(type, symbol);
    if (params) typeDesc.typeParams = params;
    if (binding.description && /@nonabstract\b/.test(binding.description)) {
      binding.description = binding.description.replace(
        /\s*@nonabstract\b/,
        ''
      );
      if (typeDesc.type == 'class') {
        ...
        if (typeDesc.instanceProperties)
          for (const name in typeDesc.instanceProperties)
  ...
  getEnumType(symbol: Symbol): BindingType {
    const properties: { [name: string]: Item } = {};
    this.gatherSymbols(
      (decl(symbol) as EnumDeclaration).members.map(
        (member) => this.tc.getSymbolAtLocation(member.name)!
      ),
      properties
    );
  ...
  getType(type: Type, forSymbol?: Symbol): BindingType {
    if (
      type.aliasSymbol &&
      !(forSymbol && forSymbol.flags & SymbolFlags.TypeAlias) &&
      this.isAvailable(type.aliasSymbol)
    )
    ...
    if (type.flags & TypeFlags.TemplateLiteral)
      return {
        ...
        typeArgs: (type as TemplateLiteralType).types.map((t) =>
          this.getType(t)
        ),
      };
    ...
    if (type.flags & TypeFlags.BooleanLiteral)
      return { type: (type as any).intrinsicName };
    if (type.flags & TypeFlags.Literal)
      return { type: JSON.stringify((type as LiteralType).value) };
    ...
    if (type.flags & TypeFlags.UnionOrIntersection) {
      ...
      if (forSymbol && (decl = maybeDecl(forSymbol))) {
        const typeNode = (decl as VariableDeclaration).type;
        if (
          typeNode &&
          (typeNode.kind == SyntaxKind.UnionType ||
            typeNode.kind == SyntaxKind.IntersectionType)
        )
          types = (typeNode as UnionOrIntersectionTypeNode).types.map((node) =>
            this.tc.getTypeAtLocation(node)
          );
      }
      const union = type.flags & TypeFlags.Union;
      let args = types.map((type) =>
        type.flags & TypeFlags.Void ? { type: 'undefined' } : this.getType(type)
      );
      // If both true and false occur in the union, combine them into boolean
      if (
        union &&
        args.some((a) => a.type == 'true') &&
        args.some((a) => a.type == 'false')
      )
        args = [{ type: 'boolean' }].concat(
          args.filter((a) => a.type != 'true' && a.type != 'false')
        );
      // Move null and undefined types to the end
      for (const tp of ['null', 'undefined']) {
        const index = args.findIndex((a) => a.type == tp);
    ...
    if (type.flags & TypeFlags.TypeParameter) {
      const name = type.symbol.name,
        found = this.typeParams.find((p) => p.name == name);
      if (
        !found &&
        maybeDecl(type.symbol)?.parent?.kind != SyntaxKind.InferType
      )
    ...
    if (type.flags & TypeFlags.Index) {
      return {
        type: 'keyof',
        typeArgs: [this.getType((type as IndexType).type)],
      };
    }
    ...
    if (type.flags & TypeFlags.IndexedAccess) {
      return {
        type: 'indexed',
        typeArgs: [
          this.getType((type as IndexedAccessType).objectType),
    ...
    if (type.flags & TypeFlags.Conditional) {
      const { root } = type as ConditionalType;
      return {
        type: 'conditional',
        typeArgs: [
          this.getType(root.checkType),
          this.getType(root.extendsType),
    ...
    if (type.flags & TypeFlags.Object) {
      const objFlags = (type as ObjectType).objectFlags;

      if (forSymbol && forSymbol.flags & SymbolFlags.Class)
        return this.getClassType(type as ObjectType);
      if (forSymbol && forSymbol.flags & SymbolFlags.Interface)
        return this.getObjectType(type as ObjectType, forSymbol);

      if (
        objFlags & (ObjectFlags.Reference | ObjectFlags.Interface) &&
        type.symbol &&
      ...
      if (objFlags & ObjectFlags.Reference) {
        ...
        if (
          ...
          (target as ObjectType).objectFlags & ObjectFlags.Tuple
        )
          return {
            type: 'tuple',
            typeArgs: (type as TypeReference).typeArguments!.map((t) =>
              this.getType(t)
            ),
          };
      }
      ...
      if (objFlags & ObjectFlags.Mapped) {
        ...
        if (objFlags & ObjectFlags.Instantiated) {
          const decl = forSymbol && maybeDecl(forSymbol);
          if (
            decl?.kind == SyntaxKind.TypeAliasDeclaration &&
            (decl as TypeAliasDeclaration).type.kind == SyntaxKind.TypeReference
          ) {
            const ref = (decl as TypeAliasDeclaration)
              .type as TypeReferenceNode;
            const name = this.tc.getTypeAtLocation(ref.typeName);
            const args = (ref.typeArguments || []).map((n) =>
              this.tc.getTypeAtLocation(n)
            );
        ...
        const decl = maybeDecl(type.symbol) as MappedTypeNode,
          innerType = decl && decl.type;
        const typeParam =
          decl && decl.typeParameter
            ? this.getTypeParam(decl.typeParameter)
            : null;
        const cx = typeParam ? this.addParams([typeParam]) : this;
        const result: BindingType = {
          type: typeParam ? 'mapped' : 'Object',
          typeArgs: [
            innerType
              ? cx.getType(this.tc.getTypeAtLocation(innerType))
              : { type: 'any' },
          ],
        };
      ...
      if (call.length)
        return {
          type: 'Function',
          signatures: call.map((c) => this.getCallSignature(c, 'function')),
        };
      ...
      if (
        ...
        return {
          type: 'typeof',
          typeArgs: [this.getReferenceType(type.symbol)],
        };

      return this.getObjectType(
        type as ObjectType,
        objFlags & ObjectFlags.Interface ? type.symbol : undefined
      );
    }
    ...
    let maybePath = '';
    let maybeD,
      maybeS = forSymbol || type.symbol;
    if ((maybeD = maybeS && maybeDecl(maybeS))) {
      const pos = getLineAndCharacterOfPosition(
        maybeD.getSourceFile(),
        maybeD.pos
      );
      maybePath = ` at ${this.nodePath(maybeD)}:${pos.line - 1}:${
        pos.character - 1
      }`;
    }
    throw new Error(
      `Unsupported type ${this.tc.typeToString(type)} with flags ${
        type.flags
      }${maybePath}`
    );
  }
  ...
  getObjectType(type: ObjectType, interfaceSymbol?: Symbol): BindingType {
    ...
    try {
      const out: BindingType = {
        type: interfaceSymbol ? 'interface' : 'Object',
      };

      const call = type.getCallSignatures(),
        ctor = type.getConstructSignatures();
      let props = type.getProperties();
      let strIndex = type.getStringIndexType(),
        numIndex = type.getNumberIndexType(),
        indexItem: Item | undefined | null;
      const intDecl = interfaceSymbol && maybeDecl(interfaceSymbol);
      let members: readonly TypeElement[] | undefined;
      if (intDecl && isInterfaceDeclaration(intDecl)) {
        const declared = intDecl.members
          .filter((member) => member.name)
          .map((member) => this.tc.getSymbolAtLocation(member.name!)!.name);
        ...
        if (intDecl.heritageClauses && intDecl.heritageClauses.length)
          out.implements = intDecl.heritageClauses[0].types.map((node) =>
            this.getType(this.tc.getTypeAtLocation(node))
          );
      }
      ...
      if (strIndex || numIndex) {
        if (!members) {
          const sym = type.getSymbol(),
            decl = sym && maybeDecl(sym);
          if (decl && decl.kind == SyntaxKind.TypeLiteral)
            members = (decl as TypeLiteralNode).members;
        }
        let indexSym: Symbol | undefined;
        if (members)
          for (const m of members)
            if (m.kind == SyntaxKind.IndexSignature)
              indexSym = (m as any).symbol;
        if (indexSym) {
          indexItem = this.extend(strIndex ? 'string' : 'number').itemForSymbol(
            indexSym,
            'property'
          );
  ...
  getClassType(type: ObjectType): BindingType {
    const out: BindingType = { type: 'class' };
    ...
    if (classDecl.heritageClauses) {
      for (const heritage of classDecl.heritageClauses) {
        const parents = heritage.types.map((node) => {
          const type = this.tc.getTypeAtLocation(node);
          for (const sym of type.getProperties()) parentProps.push(sym.name);
    ...
    const definedProps: string[] = [],
      definedStatic: string[] = [],
      ctors: Declaration[] = [];
    for (const member of classDecl.members) {
      const symbol = this.tc.getSymbolAtLocation(member.name || member)!;
      if (member.kind == SyntaxKind.Constructor) {
        ctors.push(member);
        for (const param of (member as ConstructorDeclaration).parameters) {
          if (
            getCombinedModifierFlags(param) &
            (ModifierFlags.Public | ModifierFlags.Readonly)
          )
    ...
    for (const ctor of ctors) {
      const signature = type
        ...
        .find((sig) => sig.getDeclaration() == ctor);
      if (
        !signature ||
        getCombinedModifierFlags(ctor) &
          (ModifierFlags.Private | ModifierFlags.Protected)
      )
        continue;
      const item: Binding & BindingType = {
        kind: 'constructor',
      ...
      ctorSignatures.push(
        this.extend('constructor', '.').getCallSignature(
          signature,
          'constructor',
          true
        )
    ...
    const ctorType = type.getConstructSignatures()[0];
    if (ctorType) {
      const protoProps = ctorType
        .getReturnType()
        .getProperties()
        .filter((prop) => definedProps.includes(prop.name));
      const instanceObj = this.gatherSymbols(
        protoProps,
        undefined,
        undefined,
        (name, value) => {
    ...
    const props = type
      .getProperties()
      .filter((prop) => definedStatic.includes(prop.name));
    const propObj = this.gatherSymbols(props, undefined, '^');
    if (propObj) out.properties = propObj;
  ...
  getReferenceType(
    ...
    const result: BindingType = { type: symbol.name };
    const typeSource = this.nodePath(decl(symbol));
    if (!isBuiltin(typeSource)) result.typeSource = typeSource;
    if (typeArgs) {
      const targetParams = arityType
        ? (arityType as TypeReference).target.typeParameters
        : null;
      if (arityType)
        typeArgs = typeArgs.slice(0, targetParams ? targetParams.length : 0);
      if (typeArgs.length) {
        const args = typeArgs.map((arg) => this.getType(arg));
        ...
        if (targetParams) {
          ...
          for (let i = targetParams.length - 1; i >= 0; i--) {
            const deflt = targetParams[i].getDefault();
            if (!deflt) break;
            if (!cx)
              cx = this.addParams(
                args.map((a, i) =>
                  Object.assign(
                    {
                      ...
                      id: String(i),
                      kind: 'typeparam' as const,
                    },
                    a
                  )
                )
              );
            const compare = cx.getType(deflt);
  ...
  getParams(signature: Signature): Param[] {
    return signature.getParameters().map((param) => {
      let cx = this.extend(param),
        optional = false,
        type = cx.symbolType(param);
      const decl = param.valueDeclaration as ParameterDeclaration | undefined;
      ...
      if (decl)
        this.addSourceData(
          ...
          result,
          !(
            getCombinedModifierFlags(decl) &
            (ModifierFlags.Public | ModifierFlags.Readonly)
          )
        );
      const deflt: Node = decl && (decl as any).initializer;
      if (deflt)
        result.default = deflt
          .getSourceFile()
          .text.slice(deflt.pos, deflt.end)
          .trim();
      if (deflt || optional) result.optional = true;
      if (decl && decl.dotDotDotToken) result.rest = true;
      if (
        (param.valueDeclaration as NamedDeclaration).name!.kind ==
  ...
  getTypeParams(decl: Node): Param[] | null {
    const params = (decl as any).typeParameters as TypeParameterDeclaration[];
    return !params
      ? null
      : params.reduce(
          ([res, cx]: [Param[], Context], param): [Param[], Context] => {
            const p = cx.getTypeParam(param);
  ...
  getTypeParam(param: TypeParameterDeclaration): Param {
    const sym = this.tc.getSymbolAtLocation(param.name)!;
    const localCx = this.extend(sym);
    const result: Param = {
      type: 'typeparam',
      kind: 'typeparam',
      name: sym.name,
    ...
    let constraint = getEffectiveConstraintOfTypeParameter(param),
      type;
    ...
    if (
      constraint &&
      constraint.kind == SyntaxKind.TypeOperator &&
    ...
    if (param.default)
      result.default = param
        .getSourceFile()
        .text.slice(param.default.pos, param.default.end)
        .trim();
    return result;
  }
  ...
  getCallSignature(
    ...
    const typeParams =
      ...
      this.getTypeParams(signature.getDeclaration());
    const out = { type } as CallSignature;
    if (typeParams) {
  ...
  isExternal(symbol: Symbol) {
    const decl = maybeDecl(symbol);
    if (!decl) return true;
    const path = resolve(decl.getSourceFile().fileName);
    return (
      !path.startsWith(this.basedir + sep) ||
      /\bnode_modules\b/.test(path.slice(this.basedir.length))
    );
  }
}
...
function decl(symbol: Symbol) {
  const result = maybeDecl(symbol);
  if (!result)
    throw new Error(
      `No declaration available for symbol ${symbol.escapedName}`
    );
  return result;
}
...
function compareSymbols(a: Symbol, b: Symbol) {
  const da = maybeDecl(a),
    db = maybeDecl(b);
  if (!da) return db ? -1 : 0;
  if (!db) return 1;
  const fa = da.getSourceFile().fileName,
    fb = db.getSourceFile().fileName;
...
function compareTypes(
  a: BindingType,
  b: BindingType,
  paramMap: { [id: string]: BindingType }
) {
  while (b.typeParamSource && paramMap[b.typeParamSource])
    b = paramMap[b.typeParamSource];
  if (
    a.type != b.type ||
    a.typeSource != b.typeSource ||
    a.typeParamSource != b.typeParamSource ||
    !a.properties != !b.properties ||
    a.instanceProperties ||
    b.instanceProperties ||
  ...
  if (a.properties) {
    const aK = Object.keys(a.properties),
      bK = Object.keys(b.properties!);
    if (
      aK.length != bK.length ||
      !aK.every(
        (k) =>
          b.properties![k] ||
          compareTypes(a.properties![k], b.properties![k], paramMap)
      )
    )
      return false;
  }
  ...
  if (
    ...
    (a.signatures.length != b.signatures!.length ||
      !a.signatures.every((s, i) =>
        compareSignature(s, b.signatures![i], paramMap)
      ))
  )
    return false;
  return true;
...
function getComments(node: Node) {
  ...
  while (pos < text.length) {
    ...
    if (ch === 47) {
      ...
      if (nextCh === 47) {
        const doc = text.charCodeAt(pos + 2) == 47;
        const start = (pos += doc ? 3 : 2);
        while (pos < text.length && !isLineBreak(text.charCodeAt(pos))) pos++;
        if (doc) add(text.slice(start, pos));
      } else if (nextCh === 42) {
        // asterisk
        const doc = text.charCodeAt(pos + 2) == 42,
          start = pos + (doc ? 3 : 2);
...
function stripComment(lines: string[]) {
  for (var head, i = 1; i < lines.length; i++) {
    ...
    if (lineHead != line) {
      if (head == null) {
        ...
        while (
          same < head.length &&
          head.charCodeAt(same) == lineHead.charCodeAt(same)
        )
          ++same;
  ...
  if (head != null) {
    const startIndent = /^\s*/.exec(lines[0])![0];
    const trailing = /\s*$/.exec(head)![0];
    const extra = trailing.length - startIndent.length;
  ...
  outer: for (var i = 0; i < lines.length; i++) {
    ...
    if (i == 0 && head != null) {
      for (let j = 0; j < head.length; j++) {
        const found = line.indexOf(head.slice(j));
        if (found == 0) {
    ...
    if (head == null || i == 0) lines[i] = line.replace(/^[\s\*]*/, '');
    else if (line.length < head.length) lines[i] = '';
    else lines[i] = line.slice(head.length);
  }
...
export function gatherMany(
  specs: readonly GatherSpec[]
): readonly { [name: string]: Item }[] {
  const filenames = specs.map((s) => s.filename);
  const configPath = findConfigFile(filenames[0], sys.fileExists);
  const host = createCompilerHost({});
  const options = configPath
    ? getParsedCommandLineOfConfigFile(configPath, {}, host as any)!.options
    : {};
  const program = createProgram({ rootNames: filenames, options, host });
  const tc = program.getTypeChecker();
  return specs.map(({ filename, basedir }) => {
    const items: { [name: string]: Item } = Object.create(null);
    const sourceFile = program.getSourceFile(filename);
    if (!sourceFile) throw new Error(`Source file "${filename}" not found`);
    const fileSymbol = tc.getSymbolAtLocation(sourceFile);
    if (!fileSymbol)
      throw new Error(`No symbol for file "${filename}" (no exports?)`);
    const exports = tc.getExportsOfModule(fileSymbol);
    ...
    for (let i = 0; i < closedExports.length; i++) {
      const sym = closedExports[i],
        alias = sym.flags & SymbolFlags.Alias ? tc.getAliasedSymbol(sym) : null;
